...
Meteor.methods({
    ...
    findDiffdx: function (doc) {
        //console.log(doc)
        let arr = []
        if(doc.physicalFindings  )
            arr = arr.concat ( Differentials.find({'finding_id':{ $in: doc.physicalFindings }}).fetch() )
        
        console.log(arr)
        if(doc.labFindings  )
            arr = arr.concat ( Differentials.find({'finding_id':{ $in: doc.labFindings }}).fetch() )

        //console.log(arr)
        return arr
    },

    setupFaciltiy: function (doc) {
        // wd = { 'name': 'General 2nd Floor', price: 400 }
        // rm = { name: '' }
        // beds = _.times(8, String);
        
        let prefix = parseInt(doc.prefix) != NaN ? parseInt(doc.prefix): doc.prefix+ "-";

        utils.taskDbInsert(Wards, doc.ward)
            .chain(x => utils.taskDbInsert(Rooms, { name: '__', ward: x }))
            .chain(x => {
                for( i=1; i <= doc.numberOfRooms ;i++)
                    Beds.insert({ name: 'B-' + ( prefix + i), room: x })
                    //utils.taskDbInsert(Beds, { name: 'B-' + ( doc.prefix + i), room: x })
                    //.fork(console.error, //console.log)
            }).fork(
            err => { console.error(err); throw new Meteor.Error(`Error on  insert: ${err} `) },
            succ =>{ 
